...
import { resolveFolderIcon, resolveIcon } from '../../lib/material-icons.mjs'
...
async function getFileIcon(fileName: string): Promise<Element> {
  const iconClass = await resolveIcon(fileName, null)()
  const icon = h('span', { class: ['tree-icon', iconClass].filter(Boolean) as string[] })
  if (iconClass?.startsWith('i-icons:'))
...
async function getFolderIcon(folderName: string): Promise<Element> {
  ...
  return h('span', {}, [
    h('span', { 'class': ['tree-icon', 'icon-closed', closedClass], 'data-icon': closedClass }),
    h('span', { 'class': ['tree-icon', 'icon-open', openClass], 'data-icon': openClass }),
  ])
}
